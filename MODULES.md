...
> 产品: StablePulse — 稳定币行业上下文引擎 + 自动化周报
> 定位: **自动化事实关联引擎，email-first**。核心差异化: AI 为每条事实自动匹配历史可比数据。邮件是产品，Web 是镜像+长尾事实检索。
> 目标用户: **VC 合伙人**（1-5 人内部工具）
> 架构核心: 原子事实 + Context Engine (参考知识库 + 历史事实检索 + AI 结构化对比)
>
> **AI 边界 (V11: 零意见 not 零判断)**:
> 禁止: 预测、评价、建议、"值得关注"、"可能"、"前景"
> 允许: 结构化事实对比 (e.g. "Coinbase S-1→IPO 用时 118 天")、时间线、量化背景
> AI = 上下文关联工具 (给定候选材料中选择最相关的，组成对比句)
>
> **Context Engine 三阶段 Pipeline**:
> Stage 1: 事件分类 (纯规则匹配，不用 AI)
> Stage 2: 候选检索 (参考知识库 + DB 历史事实)
> Stage 3: AI 生成 + 确定性验证 (必须含数字/日期，禁止主观词)
>
> **三层渐进式邮件 (V11)**:
> Layer 1 (2s): 市值数字行 + one-liner
> Layer 2 (30s): 3 narratives (起点/上周/本周/上下文区块/时间线)
> Layer 3 (2min): 5 signals by category (市场结构/产品动态/链上数据), 每条含 inline 上下文
> Footer: 极简数据来源
>
> **信号分类 (V11, 非重要性排序)**:
> 市场结构 (IPO/并购/融资/监管) | 产品动态 (发布/集成/合作) | 链上数据 (市值/TVL/交易量)
