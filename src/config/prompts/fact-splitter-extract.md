...
如果原文中某个**具名人物**表达了观点、判断或预测，**必须提取为 quote 类型**，并在 content 中清晰标注归因。

规则：
1. **有具名来源的观点** → 提取为 quote，content 格式必须为 "**[人名 + 头衔/身份]** 表示/认为/指出：'...'"
2. **无具名来源的观点** → 禁止提取（如"分析师认为"、"市场预期"）
3. **作者自己的分析** → 禁止提取

示例：
- ✅ "Circle CEO Jeremy Allaire 认为：'稳定币将在5年内取代大部分跨境汇款'"（quote — 有具名来源 + 身份）
- ✅ "美联储主席 Jerome Powell 表示：'我们正在密切关注稳定币的系统性风险'"（quote）
- ❌ "业内人士认为稳定币市场将继续增长" ← 无具名来源
- ❌ "这意味着Circle的IPO前景看好" ← 作者推测

**身份标注要求**：content 中人名后必须注明身份/头衔（如 CEO、CFO、联合创始人、参议员等），使读者不需要查询就能理解此人为何有资格发表该观点。
