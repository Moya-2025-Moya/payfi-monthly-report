...
- "分析师认为..." ← 只有泛称，没有具名来源
- "稳定币正在扩展到传统金融领域" ← 空洞的趋势概括，没有具体主语/动作/数据
- "跨境支付领域正在快速发展" ← 同上，纯标题式概括
- "监管环境正在逐步完善" ← 同上，毫无信息量

**空洞概括性陈述检测规则**：如果一条 content 删掉后读者不会损失任何具体信息（不知道谁做了什么、数字是多少、什么时候发生），那就是空洞概括，禁止提取。合格的事实必须包含至少一个具体要素：具体实体+具体动作、具体数据、具体时间、具体人物发言。
...
4. 删掉这条 content 后读者会损失什么具体信息？→ 如果答案是"什么都不损失"，这就是空洞概括，不提取
