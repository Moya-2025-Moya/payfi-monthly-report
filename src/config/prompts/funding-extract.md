...
1. 只提取明确的融资事件（有主语 + "融资/raised/funding/secured/invested" 等动词）
2. 不提取：收购(M&A)、上市(IPO)、代币销售(token sale)、空投
3. 战略投资（如 "Tether 投资 Ark Labs 5.2亿"）→ 提取，round 填 "Strategic"
4. 同一篇文章中可能有多个融资事件（综述类文章）
5. 金额转换：$45M = 45, $1.2B = 1200, $500K = 0.5, 未披露 = null
6. 金额未披露的融资事件也要提取（amount = null），只要有明确的融资主体和动作
7. 如果文章中没有融资事件，返回空数组 `[]`
8. 如果原文为空或无法解析，返回空数组 `[]`
