    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-end gap-3 p-4 rounded-lg border" style={{ borderColor: 'var(--border)' }}>
        <div className="flex-1 min-w-[160px]">
          <label className="block text-xs font-medium mb-1" style={{ color: 'var(--muted-fg)' }}>Week A (from)</label>
          <input
            type="text"
            value={weekA}
        ...
        <div className="flex-1 min-w-[160px]">
          <label className="block text-xs font-medium mb-1" style={{ color: 'var(--muted-fg)' }}>Week B (to)</label>
          <input
            type="text"
            value={weekB}
      ...
      {error && (
        <p className="text-sm px-3 py-2 rounded" style={{ background: '#fee2e2', color: '#991b1b' }}>{error}</p>
      )}
      ...
      {!loading && diff ? (
        ...
        <Card className="text-center py-8">
          ...
          <p className="text-sm" style={{ color: 'var(--muted-fg)' }}>Diffs are computed weekly between snapshots. Check back after the pipeline runs, or trigger it manually in Settings.</p>
        </Card>
      )}
    </div>
  )
