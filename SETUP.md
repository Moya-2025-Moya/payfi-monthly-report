...
## 3. DeFiLlama & CoinGecko (链上数据 + 融资数据) — 免费
...
1. 打开 https://www.coingecko.com/en/api → "Get Your API Key"
2. **Demo plan** 免费（30 calls/min）
3. 注册后获取 key → 填入 `COINGECKO_API_KEY`
4. 如果不需要可以留空，DeFiLlama 作为主数据源够用

---

## 4. CryptoRank (融资数据交叉验证) — 免费

1. 打开 https://cryptorank.io/api → 注册
2. 免费层有调用限额，对我们每日一次采集够用
3. 获取 key → 填入 `CRYPTORANK_API_KEY`
4. 可选：如果 DeFiLlama /raises + 新闻提取已经够用，可以不注册

测试：
```bash
curl "https://api.cryptorank.io/v1/funding-rounds?api_key=YOUR_KEY&limit=10"
```

---

## 5. free-crypto-news (新闻) — 免费

开源免费 API，无需注册。cc c

测试：
```bash
curl "https://free-crypto-news.p.rapidapi.com/news"
```

注意：可能需要通过 RapidAPI 订阅免费层。如果有限制，RSS feeds 作为主力新闻源完全够用。

---

## 6. SEC EDGAR (Filing 数据) — 免费
...
## 7. twitterapi.io (Twitter 数据) — $29/月
...
在 twitterapi.io Dashboard 中添加要追踪的账号。优先添加这6个（Starter限额）：
...
后续升级套餐可以添加 `config/twitter-accounts.ts` 中的更多账号。

### 特性

- 实时流式推送（Webhook/WebSocket）
- ~1.2s 平均延迟
- 99.9% uptime
- 历史推文访问
